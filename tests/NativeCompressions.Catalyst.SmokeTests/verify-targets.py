...
feed, output = map(lambda p: Path(p).resolve(), sys.argv[1:3])
output.mkdir(parents=True, exist_ok=True)
arch = sys.argv[3] if len(sys.argv) > 3 else "arm64"
assert arch in ("arm64", "x64")
package = f"NativeCompressions.LZ4.Runtime.maccatalyst-{arch}"
check_target = "CheckNativeCompressionsLz4Catalyst" + ("Arm64" if arch == "arm64" else "X64")
...
base = dict(TargetFramework="net10.0-maccatalyst", TargetPlatformIdentifier="maccatalyst", RuntimeIdentifier=f"maccatalyst-{arch}", OutputType="Exe")
cases = {
    "arm64": ({"RuntimeIdentifier": "maccatalyst-arm64"}, int(arch == "arm64")),
    "x64": ({"RuntimeIdentifier": "maccatalyst-x64"}, int(arch == "x64")),
    ...
    "universal-inner": ({"RuntimeIdentifiers": "maccatalyst-arm64;maccatalyst-x64"}, 1),
...
for name, (overrides, count) in cases.items():
    ...
    result = subprocess.run(["dotnet", "msbuild", str(project), "-nologo", "-v:q", "-t:" + check_target, "-getItem:NativeReference"], text=True, encoding="utf-8", errors="replace", capture_output=True)
...
project.write_text((output / f"{arch}.proj").read_text(encoding="utf-8").replace(str(targets), str(missing / targets.name)), encoding="utf-8")
result = subprocess.run(["dotnet", "msbuild", str(project), "-nologo", "-t:" + check_target], text=True, encoding="utf-8", errors="replace", capture_output=True)
...
for mode, ids in {
    ...
    result = subprocess.run(["dotnet", "msbuild", str(probe), "-nologo", "-v:q", "-t:" + check_target, "-getItem:NativeReference"], text=True, encoding="utf-8", errors="replace", capture_output=True)
    assert result.returncode == 0, result.stdout + result.stderr
...
for rid, expected in {
    ...
    "maccatalyst-x64": ["runtimes/maccatalyst-x64/native/liblz4.a"],
