...
Initial Catalyst `libopenzl.a` files are not present yet. After CI succeeds, collect `catalyst-openzl/<arch>/libopenzl.a` and provenance from `catalyst-phase3-lz4-zstandard-openzl-<arch>` and place the verified archives under `src/NativeCompressions.OpenZL.Runtime/runtimes/maccatalyst-<arch>/native/`. Do not mark OpenZL or three-library Catalyst execution complete until the ARM64/x64 app runs pass. Production OpenZL native-update integration and .NET 11 remain subsequent work.